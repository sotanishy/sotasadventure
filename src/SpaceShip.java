...
public class SpaceShip {
	...
	public ArrayList<Enemy> deployEnemies(ArrayList<Enemy> enemies, float elapsedTime, int enemyWidth, int enemyHeight) {
		if (elapsedTime - deployedTime > DEPLOY_INTERVAL) {
			deployedTime = elapsedTime;

			Enemy enemy = new Enemy(enemyWidth, enemyHeight);
			enemy.init((position.x + width / 2) / Map.TILE_SIZE, (position.y + height) / Map.TILE_SIZE);
			enemies.add(enemy);
		}
		return enemies;
	}
